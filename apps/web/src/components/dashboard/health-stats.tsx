...
export function HealthStats() {
    ...
    const stats = useMemo(() => {
        ...
        const monthly = appointments?.filter(a => new Date(a.scheduledAt) >= startOfMonth) ?? [];
        const upcoming = appointments?.filter(a => {
            const d = new Date(a.scheduledAt);
            return d >= now && a.status !== 'completed' && a.status !== 'cancelled';
        }) ?? [];
        const next7Days = upcoming.filter(a => new Date(a.scheduledAt) <= endOfNext7Days);
        ...
        if (lastCompleted) {
            const diffMs = now.getTime() - new Date(lastCompleted.scheduledAt).getTime();
        ...
        return [
            {
                ...
                unit: 'this month',
                icon: UserCheck,
                trend: {value: `${completed.length} completed`, type: 'neutral' as const},
            },
            {
            ...
            },
            {
                ...
                unit: lastCompleted
                    ? new Date(lastCompleted.scheduledAt).toLocaleDateString('en-US', {month: 'short', day: 'numeric'})
                    : 'no records',
                icon: Calendar,
                trend: {value: lastCompleted ? 'Completed' : 'No records', type: 'positive' as const},
            },
            {
