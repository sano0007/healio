...
export function UpcomingAppointments() {
  ...
  const upcoming = (appointments ?? [])
      .filter((apt) => apt.status === "confirmed" || apt.status === "pending" || apt.status === "awaiting_payment")
      .slice(0, 4);
    ...
    return (
        <div className="bg-white rounded-[2rem] p-8 border border-gray-100 shadow-sm">
          <div className="flex items-center gap-3 mb-6">
            <div
                className="w-10 h-10 rounded-xl bg-emerald-50 flex items-center justify-center border border-emerald-100">
          ...
          >
            <div className="flex items-center gap-4 flex-1">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 font-medium">
                  {appt.doctor?.specialty || "General Physician"} • {(appt.type || "video")}
                </p>
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto">
              {appt.type === "video" && appt.status === "confirmed" && (
                  <Link href={`/consultations/${appt._id}`} className="flex-1 sm:flex-none">
              ...
              <Link href={`/appointments/${appt._id}`} className="flex-1 sm:flex-none">
