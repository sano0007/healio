...
export type NotificationType = "appointment" | "payment" | "consultation" | "prescription" | "verification";
...
export function NotificationListItem({ type, title, description, timestamp, isRead, link }: NotificationListItemProps) {
    ...
    <Link href={link}>
      <motion.div 
        ...
        className={cn(
          ...
          isRead ? "border-gray-50 opacity-80" : "border-brand-light/20 shadow-sm border-l-4 border-l-brand-dark"
        )}
      >
        {/* Type Icon */}
        <div className={cn("w-12 h-12 rounded-2xl flex items-center justify-center border shrink-0", config.color)}>
           {config.icon}
        </div>
        ...
        <div className="flex-1 space-y-1.5 min-w-0">
           <div className="flex items-center justify-between gap-4">
              <h4 className={cn("text-sm font-bold tracking-tight truncate", isRead ? "text-gray-600" : "text-brand-black")}>
                {title}
              </h4>
