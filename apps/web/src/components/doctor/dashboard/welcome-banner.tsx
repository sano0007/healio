...
export function WelcomeBanner() {
  ...
  const [localStatus, setLocalStatus] = useState<'online' | 'busy' | 'offline'>(
    'online',
  );
  const currentStatus =
    (doctor?.status as 'online' | 'busy' | 'offline') || localStatus;
  ...
  const upcomingAppointments = todayAppointments
    .filter(
      (a) => a.status !== 'completed' && new Date(a.scheduledAt) > currentTime,
    )
    .sort(
      (a, b) =>
  ...
  if (firstAppointment) {
    const diff =
      new Date(firstAppointment.scheduledAt).getTime() - currentTime.getTime();
          ...
          <div className="space-y-4">
            ...
            <p className="text-lg text-brand-light/60 font-medium max-w-lg leading-relaxed">
              ...
              {minutesUntilFirst !== null && minutesUntilFirst > 0 && (
                <>
                  {' '}
                  Your first patient is arriving in{' '}
                  <span className="text-white font-bold">
                    {minutesUntilFirst} minutes
                  </span>
                  .
                </>
              )}
              ...
              <>
                <div className="flex -space-x-3">
                  {todayAppointments.slice(0, 4).map((apt, i) => (
                    <div
                      key={i}
                      className="w-10 h-10 rounded-full border-2 border-brand-dark bg-gray-100 overflow-hidden shadow-xl flex items-center justify-center"
                    >
                ...
                >
                  ...
                  <div
                    className={cn(
                      ...
                      currentStatus === s
                        ? 'scale-125'
                        : 'scale-100 group-hover/btn:scale-110',
                    )}
                  />
                </button>
              ))}
