...
export function TodaySchedule() {
  ...
  const leftCount = todayAppointments.filter(
    ...
    <div className="bg-white rounded-[3rem] border border-gray-100 p-8 lg:p-12 shadow-sm space-y-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-1">
          ...
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-5">
            {leftCount} Patients left for the day
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <Button
            variant="outline"
            className="h-10 rounded-xl text-[10px] font-bold uppercase tracking-widest border-gray-100 italic"
          ...
          <Button
            variant="dark"
            className="h-10 bg-brand-light text-brand-dark hover:bg-white rounded-xl text-[10px] font-bold uppercase tracking-widest shadow-lg shadow-brand-dark/5"
      ...
      <div className="relative">
        <div
          className="absolute left-[39px] top-6 bottom-6 w-px bg-gradient-to-b from-gray-100 via-gray-100 to-transparent dashed"
          style={{
            backgroundImage:
              'linear-gradient(to bottom, transparent 50%, #F3F4F6 50%)',
        ...
        <div className="space-y-12">
          {todayAppointments.length === 0 ? (
            ...
            todayAppointments.map((session, index) => {
              ...
              const patientName =
                patient?.name || `Patient ${session.patientId.slice(0, 6)}`;
                ...
                >
                  ...
                  >
                    <div className="flex items-center gap-4 min-w-[200px]">
                      <div className="w-16 h-16 rounded-2xl bg-gray-100 flex items-center justify-center text-gray-300 border-2 border-white shadow-sm">
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      <Button
                        variant={
                          session.status === 'completed' ? 'outline' : 'dark'
                        }
                        className={cn(
                        ...
                        onClick={() =>
                          router.push(`/consultations/${session._id}`)
                        }
                      >
