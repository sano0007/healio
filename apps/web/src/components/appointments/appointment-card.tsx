      ...
      <div className="flex flex-col md:flex-row gap-6 md:items-center">
        ...
        <div className="flex items-center gap-4 shrink-0">
          <Avatar
            ...
            className="w-16 h-16 md:w-20 md:h-20 border-2 border-white shadow-sm"
          />
          <div>
            <div className="flex items-center gap-2 mb-1">
              <h3 className="text-base font-bold text-brand-black truncate max-w-[120px] md:max-w-full">
        ...
        <div className="flex items-center gap-3 md:pl-6 md:border-l border-gray-50">
          {appointment.status === 'confirmed' && (
            <>
              {appointment.type === 'video' ? (
                <Button
                  variant="dark"
                  className="rounded-xl px-6 h-12 text-xs font-bold gap-2 shadow-lg shadow-brand-dark/10 hover:scale-[1.02] active:scale-95 transition-all"
                >
                  Join Meeting
                ...
                <Button
                  variant="outline"
                  className="rounded-xl px-6 h-12 text-xs font-bold gap-2 border-brand-light/20 hover:bg-brand-light/5"
                >
                  View Map
          ...
          {appointment.status === 'completed' && (
            <div className="flex gap-2">
              <Button
                variant="outline"
                className="rounded-xl h-12 px-4 text-xs font-bold gap-2 border-brand-light/20"
              >
          ...
          {appointment.status === 'cancelled' && (
            <Button
              variant="dark"
              className="rounded-xl h-12 px-6 text-xs font-bold gap-2 shadow-lg shadow-brand-dark/10"
            >
              Rebook Appointment
            </Button>
          )}
          ...
          {appointment.status === 'confirmed' && (
            <div className="flex gap-2">
              <Button
                variant="ghost"
                size="icon"
                className="w-12 h-12 rounded-xl text-gray-400 hover:text-rose-500 hover:bg-rose-50 border border-transparent hover:border-rose-100"
              >
              ...
              <Button
                variant="ghost"
                size="icon"
                className="w-12 h-12 rounded-xl text-gray-400 hover:text-brand-dark hover:bg-brand-light/10 border border-transparent hover:border-brand-light/20"
              >
