        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6 mb-12">
          <div>
            ...
            <h2 className="text-3xl sm:text-4xl font-bold text-brand-black leading-snug mt-2">
              Why 30,000+ Patients<br />
              <em className="font-serif italic font-normal">choose us every year</em>
            </h2>
          </div>
          <div className="flex gap-2">
            ...
            <button
              onClick={() =>
                setTestimonialIndex(Math.min(testimonials.length - 1, testimonialIndex + 1))
              }
          ...
          {testimonials.map((test, i) => (
            <div key={i} className="testimonial-card">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <p className="font-semibold text-brand-black text-sm">{test.name}</p>
              ...
              <p className="text-sm text-gray-500 leading-relaxed">{test.desc}</p>
            </div>
          ))}
        </div>
      </div>
