  ...
  useEffect(() => {
    ...
    if (date && time) {
      setBookingData((prev) => ({
        ...prev,
        selectedDate: date,
        selectedTime: decodeURIComponent(time),
      }));
    }
  }, []);
    ...
    try {
      ...
      if (selectedDateStr && selectedTimeStr) {
        ...
        if (hasSlash) {
          ...
          const isPM =
            selectedTimeStr.toLowerCase().includes('pm') && timeParts[0] !== 12;
          year = dateParts[2];
          month = dateParts[0] - 1;
          day = dateParts[1];
  ...
  return (
    <div className="max-w-7xl mx-auto py-8 lg:py-12">
      ...
      <div className="px-4 mb-12">
        <h1 className="text-3xl font-bold text-brand-black mb-10 text-center">
      ...
      <div className="grid lg:grid-cols-3 gap-12 px-4 items-start">
        ...
        <div className="lg:col-span-2 bg-white rounded-[2.5rem] p-8 md:p-12 border border-gray-100 shadow-sm min-h-[600px]">
          <AnimatePresence mode="wait">
            {step === 1 && <PatientInfoStep key="step1" onNext={handleNext} />}
            {step === 2 && (
        ...
        <div className="order-first lg:order-last">
          <BookingSummarySidebar doctor={doctorDisplay} details={details} />
        </div>
      </div>
    </div>
  );
