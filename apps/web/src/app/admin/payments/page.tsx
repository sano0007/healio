...
export default function AdminPaymentsPage() {
  ...
  const exportCSV = useExportToCSV<{
    ...
    payments || [],
    `healio_payments_${new Date().toISOString().split('T')[0]}.csv`,
    [
          ...
          <tbody className="divide-y divide-gray-50">
            {payments?.map((payment) => (
              ...
              >
                ...
                <td className="px-5 py-3 text-gray-500 uppercase">
                  {payment.currency}
                </td>
                <td className="px-5 py-3">
                  <span
                    className={`px-2 py-0.5 rounded-full text-xs font-medium capitalize ${STATUS_STYLES[payment.status] || 'bg-gray-100 text-gray-600'}`}
                  >
                    {payment.status}
                  </span>
                </td>
