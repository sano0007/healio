...
export class AdminGatewayController {
  ...
  async getStats() {
    const [patients, doctors, appointments, payments] = await Promise.all([
      ...
      firstValueFrom(
        this.doctorClient.send(MSG.DOCTOR_GET_ALL_ADMIN, {}),
      ).catch(() => []),
    ...
    return {
      ...
      verifiedDoctors: (doctors as { isVerified: boolean }[]).filter(
        (d) => d.isVerified,
      ).length,
      pendingVerification: (doctors as { isVerified: boolean }[]).filter(
        (d) => !d.isVerified,
      ).length,
      ...
      successfulPayments: (payments as { status: string }[]).filter(
        (p) => p.status === 'success',
      ).length,
    };
  }
